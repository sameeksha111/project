...
let currentCharts = { status: null, priority: null };
...
async function loadDashboard() {
  ...
  if (cases && cases.length > 0) {
    ...
    const high = cases.filter(c => c.priority === 'high' && c.status === 'open').length;
  ...
  // Add charts section for staff only
  if (cases && cases.length > 0 && isStaff) {
    html += `
      <div class="charts-section">
        <h3 style="margin-top: 2rem; margin-bottom: 1rem;">📈 Analytics (Staff Only)</h3>
  ...
  // Render charts if staff and cases exist
  if (cases && cases.length > 0 && isStaff) {
    setTimeout(() => renderCharts(cases), 100);
    setTimeout(() => renderRecentActivity(cases), 100);
  }
}
...
function renderCharts(cases) {
  ...
  if (currentCharts.priority) currentCharts.priority.destroy();
  ...
  currentCharts.priority = new Chart(priorityCtx, {
    type: 'bar',
    data: {
...
function renderRecentActivity(cases) {
  ...
  let html = '';
  sorted.forEach(c => {
    const statusBadge = c.status === 'closed' ? '<span class="badge-closed">✓ Closed</span>' : '<span class="badge-open">🔴 Open</span>';
    const priorityColor = c.priority === 'high' ? '#ff6b6b' : c.priority === 'medium' ? '#ffd93d' : '#6bcf7f';
    
    html += `
      <div class="recent-item" style="border-left: 4px solid ${priorityColor};">
        <div class="recent-item-header">
          <strong>${c.caseId}</strong> - ${c.petName}
          ${statusBadge}
        </div>
        <div class="recent-item-meta">
          <small>📅 ${new Date(c.createdAt).toLocaleDateString()} | ${c.ownerName} | ${c.issueType}</small>
        </div>
      </div>
    `;
  });
  
  recentList.innerHTML = html || '<p>No cases yet</p>';
}
